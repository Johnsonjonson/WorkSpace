...
public class Analysis {
    ...
    public static int lengthArray(byte[] as){
        int number = 0;
        for (byte a : as) {
            if (a==0){
                break;
    ...
    public static String getByteToString(byte[] bytes,boolean isHex){
        try {
            if (isHex)
                return bytesToHexString(bytes);
            else
                return new String(bytes,0,lengthArray(bytes),"GBK");
    ...
    public static byte[] getBytes(String data,boolean isHex){
        byte[] buff = null;
        if (!isHex) {
            try {
