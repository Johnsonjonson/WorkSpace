...
public class CRCUtils {
    ...
    /**
     *封装并发送报文
     *这里报文格式为{报文总长度[2字节]，Crc32校验码[4字节]，ver版本信息[1字节]，cmd指令[1字节]，data数据内容[若干字节]}
     */
    private static void pack(OutputStream out, byte cmd, byte[] data, int len) {
        try {
            //先得到报文总长度，如上介绍的格式 字节数相加
            short totalSize = (short) (2 + 4 + 1 + 1 + len);
            //输出报文长度
            out.write(shortToByte(totalSize), 0, 2);
            //需要校验的字段
            byte[] bytes = new byte[1 + 1 + len];
            int index = 0;
            bytes[index++] = 0x05;//版本号（这里是写死的）
            bytes[index++] = cmd;//cmd指令
            //数据内容
            for (int i = 0; i < len; i++) {
                bytes[index++] = data[i];
            }
            //得到ver、cmd、data的crc32校验码（Crc32算法在最下边，网上找的.....）
            byte[] crc32 = intToBytes(getCRC32(bytes));
            //输出crc校验码
            out.write(crc32, 0, crc32.length);
            //输出ver、cmd、data
