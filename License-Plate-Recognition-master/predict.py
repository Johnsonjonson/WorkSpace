...
class CardPredictor:
	...
	def predict(self, car_pic, resize_rate=1):
		...
		for card_index,card_img in enumerate(card_imgs):
			...
			# if len(card_img.shape) == 3 or len(card_img.shape) == 4:
			# 	card_img_hsv = cv2.cvtColor(card_img, cv2.COLOR_BGR2GRAY)
			# else:
			# 	card_img_hsv = card_img
			# card_img = card_img.astype(np.float32)
