...
class Surface(ttk.Frame):
    ...
    def __init__(self, win):
        ...
        self.user_enter_time = ttk.Label(frame_right3, text="进入时间：")
        self.user_enter_time.grid(column=0, row=2, sticky=tk.W)
        self.user_exit_time = ttk.Label(frame_right3, text="离开时间：")
        self.user_exit_time.grid(column=0, row=3, sticky=tk.W)
        self.user_total_time = ttk.Label(frame_right3, text='耗时：')
        self.user_total_time.grid(column=0, row=4, sticky=tk.W)
    ...
    def parse_resrlt(self,result,file):
        ...
        try:
            ...
            self.recognizeTime = self.recognizeTime + 1
            if not self.recognizeTime%2 == 0: # 进入
                self.enter_time = datetime.datetime.now()
                self.check_car_num = self.recognize_car_num  # 等待检测车牌
                # self.user_total_time.configure(text="耗时：" + data['total_time'], state='enable')
                # self.user_exit_time.configure(text="离开时间：" + data['exit_time'], state='enable')
                self.user_enter_time.configure(text="进入时间：" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), state='enable')
            else:  # 离开
                print(self.recognize_car_num,self.check_car_num)
                if self.recognize_car_num != "" and self.check_car_num != "" and self.recognize_car_num == self.check_car_num:
                    ...
                    result = self.exit_time - self.enter_time
                    hours = int(result.seconds / 3600)
                    minutes = int(result.seconds % 3600 / 60)
                    seconds = result.seconds % 3600 % 60
                    self.user_exit_time.configure(text="离开时间：" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),state='enable')
                    ...
                    self.recognizeTime = self.recognizeTime - 1  #需要重新识别
                    self.result.configure(text="比对失败,不通过，请重新识别", background="#FF0000", state='enable')
            try:
                img = cv2.imread(file)
    ...
    def update_user_info(self,data):
        self.user_total_time.configure(text="耗时：" + data['total_time'], state='enable')
        self.user_exit_time.configure(text="离开时间：" + data['exit_time'], state='enable')
        self.user_enter_time.configure(text="进入时间：" + data['enter_time'], state='enable')
    ...
    def setCardType(self):
        request_url = 'http://129.204.232.210:8540/set'

        params = {"card_type": 0}
        headers = {'content-type': 'application/x-www-form-urlencoded'}
        response = requests.get(request_url, data=params, headers=headers)
        print(response)
